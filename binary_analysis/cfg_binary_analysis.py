...
"""
This script contains the CFG Extended Analysis filter and the CFG Analysis filter in the simple_filter() function.
Arguments:
...
class CFGAnalysis(ReductionCallback):
...
def main():
    ...
    while(counter < program_num or program_num == -1):
        ...
        dir_name = "../data_cfg/program_" + str(counter)
        ...
        if interesting:
            ...
            # reduce
            sanitizer = Sanitizer()
            rprogram = Reducer().reduce(program, CFGAnalysis(sanitizer, settings), jobs=16)
